...
var tripFuelEntryCount = 1;
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function createFuelEntry(container, context) {
            ...
            var idPrefix = context === 'trip' ? 'add_' : 'machinery_';
    ...
    <h4 class="mt-4">Fuel Information</h4>
        <div id="fuel-entries-edit-trip">
            <div class="fuel-entry row mb-3">
                ...
                <div class="col-12 col-md-5">
                    <label for="cost[]" class="form-label">Cost per Litre  (ZMW)<span class="text-danger">*</span></label>
                    <input type="number" step="0.01" name="fuels[0][cost]" class="form-control @error('fuels.0.cost') is-invalid @enderror"
                        value="{{ old('fuels.0.cost') }}" placeholder="example: 60" required>
                    @error('fuels.0.cost')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
...
<!-- ---------------------Latest and Improved Equipment trips Tab-------------------------------------------------- -->
@extends('layouts.app')
            ...
            <div class="card mt-3">
                ...
                <div class="card-body">
                    ...
                    @else
                        ...
                        @else
                            <div class="table-responsive">
    <table class="table table-bordered table-striped table-sm" id="tripsTable">
        <thead class="table-dark">
            <tr>
                <th title="Trip Dates">Dates</th>
                <th title="Kilometers">Km</th>
                <th title="Location">Loc</th>
                <th title="Driver">Driver</th>
                <th title="Material Delivered">Material</th>
                <th title="Supplier Name">Supplier</th>
                <th title="Weights (tonnes)">Weights</th>
                <th title="Costs (ZMW)">Costs</th>
                <th title="Fuel Data">Fuel</th>
                <th title="Actions">Act</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($equipment->trips as $trip)
                <tr>
                    <td>
                        ...
                        @if ($trip->return_date)
                            to {{ $trip->return_date->format('Y-m-d') }}
                        @else
                            <span class="text-muted">-</span>
                        @endif
                    </td>
                    <td>
                        @if ($trip->start_kilometers !== null && $trip->start_kilometers >= 0)
                            {{ number_format($trip->start_kilometers, 0) }}
                            @if ($trip->end_kilometers !== null && $trip->end_kilometers > 0)
                                to {{ number_format($trip->end_kilometers, 0) }}
                                ({{ number_format($trip->end_kilometers - $trip->start_kilometers, 0) }} km)
                            @else
                                <span class="text-muted">-</span>
                            @endif
                        @else
                    ...
                    <td>{{ $trip->supplier_name ?? '-' }}</td>
                    <td>
                        @if ($trip->gross_weight !== null || $trip->tare_weight !== null || $trip->net_weight !== null)
                            G: {{ $trip->gross_weight ? number_format($trip->gross_weight, 1) : '-' }}t
                            T: {{ $trip->tare_weight ? number_format($trip->tare_weight, 1) : '-' }}t
                            N: {{ $trip->net_weight && $trip->net_weight > 0 ? number_format($trip->net_weight, 1) : '-' }}t
                        @else
                            <span class="text-muted">-</span>
                        @endif
                    </td>
                    <td>
                        @php
                            $costs = [
                                ...
                                'Council' => $trip->council_fee,
                                'Weighbridge' => $trip->weighbridge,
                                'Toll Gate' => $trip->toll_gate,
                                'Other Expenses' => $trip->other_expenses
                            ];
                            $hasCosts = array_filter($costs, fn($cost) => $cost !== null && $cost > 0);
                        @endphp
                        @if ($hasCosts)
                            @foreach ($costs as $key => $cost)
                                @if ($cost !== null && $cost > 0)
                                    {{ $key }}: {{ number_format($cost, 0) }} ZMW
                                @endif
                            @endforeach
                        @else
                            <span class="text-muted">-</span>
                    ...
                    <td>
                        ...
                        @else
                            ...
                            @endphp
                            <span title="@foreach ($trip->fuels as $fuel){{ number_format($fuel->litres_added, 2) }}L {{ $fuel->refuel_location ? 'at ' . $fuel->refuel_location : '' }} {{ $fuel->cost !== null && $fuel->cost !== 0 ? '(' . number_format($fuel->cost, 2) . ' ZMW/L)' : '(-)' }}&#10;@endforeach">
                                {{ number_format($trip->fuels->sum('litres_added'), 0) }}L
                                ({{ $totalFuelCost > 0 ? number_format($totalFuelCost, 0) . ' ZMW' : '-' }})
                            </span>
                        @endif
                    </td>
                    <td>
            ...
            @empty
                <tr>
                    <td colspan="10" class="text-center text-muted">No trips recorded for this equipment.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
...
<script>
    $(document).ready(function() {
        $('#tripsTable').DataTable({
            ...
            columnDefs: [
                { targets: [6, 7], visible: false }, // Hide Weights, Costs by default
...
<script>
    ...
    $(document).ready(function() {
        let fuelEntryCount = {{ $trip->fuels->count() ?: 1 }}; // Start after existing fuels
